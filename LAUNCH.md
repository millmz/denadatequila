...
| 5 | ~~Klaviyo flows built~~ **Rebuilt Sept 22 after the email audit.** Welcome (3), Abandoned checkout (2), Added to cart (1), Post-purchase (4, on delivery), Win-back and the Behind the Bar recipe series (26 emails, one every two weeks) are built and **in Draft** until launch day. One flow is **live now**: the pre-launch confirmation ("You're on the list."), which goes off on launch morning. IDs in `email/KLAVIYO.md` | — | Flows page |
...
| 8 | **Old subscriber list imported** — Squarespace export → Klaviyo, with the property `signup_source` = `legacy` set on import. That property is what keeps them out of the pre-launch confirmation and the launch email, and gives them their own "We moved" hello on Oct 2 | you | Member count; segment *Legacy subscribers* fills |
...
| 7 | **Klaviyo flows**: Pre-launch confirmation → **Draft** (off). Then Welcome, Abandoned checkout, Added to cart, Post-purchase, Win-back, Behind the Bar → **Live**. Say "go" and Claude does all seven, or do it in Flows | you / [Claude] | 1 min |
...
| # | Item | Owner | How to check |
|---|---|---|---|
| 15 | **The pre-launch confirmation is sending now.** Anyone who signs up on the teaser gets "You're on the list." fifteen minutes later. Its footer prints the organization address, which is still blank in Klaviyo (item 5a). Fill it in today | you | Sign up on the teaser with a personal address; the email arrives with a street address in the footer |
| 16 | **Klaviyo onsite tracking on the Shopify theme.** Klaviyo → Integrations → Shopify → Onsite tracking on; then confirm the Klaviyo snippet and the Added to Cart snippet are in `theme.liquid` (Klaviyo's Shopify guide). Without it the *Added to cart* flow never fires and *Viewed Product* never records | you | Add a bottle to the cart on the store; an *Added to Cart* event shows on your Klaviyo profile |
